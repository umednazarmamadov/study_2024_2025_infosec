...
подготовим веб-приложение, которое будем сканировать. Это будет DVWA. Для этого запустила apache2 (рис. [-@fig:001]).

![Запуск apache2](image/1.png){#fig:001 width=70%}

Ввожу в адресной строке браузера адрес DVWA (рис. [-@fig:002]).
...
Проверить веб-приложение можно, введя его полный URL и не вводя порт(рис. [-@fig:004]).

![Проверить веб-приложение](image/4.png){#fig:004 width=70%}

просканировать адрес хоста и адрес порта, результаты незначительно отличаются (рис. [-@fig:005]).

![просканировать адрес хоста](image/5.png){#fig:005 width=70%}
